...
type Seat = { id: string; token: string; name: string; color: Color; lastSeen: number };
type Room = { code: string; humanCount: number; seats: Seat[]; game: Game | null; locked: Record<string, Selection>; ready: string[]; reviewed?: string[]; revealAt?: number; beforeReveal?: Game; selectionDeadline?: number; timedOut?: string[] };
...
function file(code: string) {
  if (!/^[A-Z2-9]{6}$/.test(code)) throw new Error('ルームコードは6文字です');
  return path.join(directory, `${code}.json`);
}
function save(room: Room) {
  mkdirSync(directory, { recursive: true });
  const filename = file(room.code), temporary = `${filename}.tmp`;
  writeFileSync(temporary, JSON.stringify(room), 'utf8'); renameSync(temporary, filename);
}
function read(code: string): Room {
  const filename = file(code);
  if (!existsSync(filename)) throw new Error('ルームが見つかりません。コードを確認してください');
  return JSON.parse(readFileSync(filename, 'utf8')) as Room;
}
...
export function createRoom(humanCount: number, name: unknown): Session {
  if (![1, 2, 3, 4].includes(humanCount)) throw new Error('プレイ人数を選んでください');
  const alphabet = 'ABCDEFGHJKLMNPQRSTUVWXYZ23456789';
  let code: string;
  do { code = Array.from({ length: 6 }, () => alphabet[randomInt(alphabet.length)]).join(''); } while (existsSync(file(code)));
  const own: Seat = { id: 'p0', name: nameOf(name), color: 'ruby', token: randomBytes(32).toString('hex'), lastSeen: Date.now() };
  const room: Room = { code, humanCount, seats: [own], game: null, locked: {}, ready: [] };
  save(room); return session(room, own);
}
export function joinRoom(code: string, name: unknown): Session {
  ...
  const own = { id: `p${room.seats.length}`, name: nameOf(name), color, token: randomBytes(32).toString('hex'), lastSeen: Date.now() };
...
export function getRoom(code: string, token: string): RoomView {
  const room = read(code), own = seat(room, token);
  own.lastSeen = Date.now(); enforceDeadline(room); save(room);
...
export function act(code: string, token: string, action: string, payload: unknown) {
