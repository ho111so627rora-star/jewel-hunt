...
export function Rules({ onClose }: { onClose: () => void }) {
  ...
  return <div className="modal-backdrop" onClick={onClose}><section className="modal rules" role="dialog" aria-modal="true" aria-labelledby="rules-title" onClick={e => e.stopPropagation()}>
    ...
    <p className="eyebrow">HOW TO PLAY</p><h2 id="rules-title">読み合いで、宝石を手に。</h2><p>全員で10ターン。集めた宝石の合計点が一番高いハンターの勝ちです。同点なら同率優勝！</p>
    <ol className="rule-steps"><li><b>2個を選ぶ</b><span>袋から左右に1個ずつ。宝石は空いている数字を選びます。</span></li><li><b>全員で一斉公開</b><span>自分の左と相手の右、自分の右と相手の左が対決します。左右は各自から見た向きで、相手の席が正面・右隣・左隣でも同じです。</span></li><li><b>宝石を集める</b><span>数字がそのまま得点に。最後に4色そろうと＋5点。</span></li></ol>
    ...
    <p className="fine-print">特殊サイコロは使い切り。ゾロ目で盗まれなかった宝石は袋に戻ります。宝石が完売すると、その色は1点だけ選択できます。手持ちが足りない場合は残りを出し、空ならパスします。</p>
