...
export function Lobby({ room, act, busy }: { room: RoomView; act: (action: string, data?: object) => Promise<void>; busy: boolean }) {
  ...
  return <main className="lobby"><p className="lobby-caption">ファントムジェム / プレイの準備</p><h1>席についたら、始めよう。</h1><p className="muted">{room.humanCount === 1 ? 'CPUの準備はできています。好きな色を選んで始めましょう。' : '友だちにコードを伝えて、同じルームに集まろう。'}</p>
    <div className="room-ticket"><span>招待コード</span><strong>{room.code}</strong><button className="icon-button" onClick={copy} aria-label="招待コードをコピー">{copied ? <Check /> : <Copy />}</button></div>{copied && <p role="status" className="muted">招待コードをコピーしました</p>}
