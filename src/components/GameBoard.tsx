...
export function GameBoard({ room, busy, act, revealing }: { room: RoomView; busy: boolean; act: (action: string, data?: object) => Promise<void>; revealing: number | null }) {
  ...
  return <main className="table-game"><div className="table-hud"><div className="table-turn"><span>TURN</span><b>{String(game.turn).padStart(2, '0')}</b><span>/ 10</span></div><div className="table-match"><Swords /><span>今回の相手 <b>{opponent.name}</b></span><Gem kind={opponent.color} /></div><button className={`view-button ${overhead ? 'active' : ''}`} aria-label={overhead ? '自分の視点に戻す' : '卓を上から見る'} aria-pressed={overhead} onClick={() => setOverhead(!overhead)}><Eye /><span>{overhead ? '自分の視点へ' : '卓を見渡す'}</span></button></div>
    ...
    <section className="table-stage" aria-label="4人で囲むゲーム卓">
      ...
      <div className="table-tools"><button onClick={() => setDrawer('market')}><GemIcon /><span>宝石の空き目</span></button><button onClick={() => { setInspectedPlayer(opponent.id); setDrawer('collection'); }}><Trophy /><span>みんなの持ち物</span></button><button onClick={() => setDrawer('log')}><ScrollText /><span>記録</span></button></div>
    ...
    {drawer && <Drawer title={drawer === 'market' ? '宝箱と宝石マーケット' : drawer === 'collection' ? 'みんなの宝石と使用状況' : 'このターンの記録'} onClose={() => setDrawer(null)}>{drawer === 'market' ? <><p className="drawer-note">宝箱には{(game.miningCount ?? game.miningBag.length)}個のサイコロ。採掘権を使うと、ここから1個取って手持ちに加えます。</p><Market game={game} /></> : drawer === 'collection' ? <PlayerIntel game={game} selected={inspectedPlayer} opponent={opponent.id} me={room.me} onSelect={setInspectedPlayer} /> : <div className="drawer-log">{game.phase === 'select' || countdown ? <p>全員のカップが公開されると、ここに記録が残ります。</p> : game.logs.map((log, i) => <p key={i}>{log}</p>)}</div>}</Drawer>}
  </main>;
}
