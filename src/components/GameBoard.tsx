...
import { ArrowRight, Eye, Gem as GemIcon, ScrollText, Swords, Trophy, X } from 'lucide-react';
...
export function GameBoard({ room, busy, act, revealing }: { room: RoomView; busy: boolean; act: (action: string, data?: object) => Promise<void>; revealing: number | null }) {
  ...
  const [drawer, setDrawer] = useState<'market' | 'collection' | 'log' | null>(null), [overhead, setOverhead] = useState(false);
  ...
  return <main className="table-game"><div className="table-hud"><div className="table-turn"><span>TURN</span><b>{String(game.turn).padStart(2, '0')}</b><span>/ 10</span></div><div className="table-match"><Swords /><span>今回の相手 <b>{opponent.name}</b></span><Gem kind={opponent.color} /></div><button className={`view-button ${overhead ? 'active' : ''}`} aria-label={overhead ? '自分の視点に戻す' : '卓を上から見る'} aria-pressed={overhead} onClick={() => setOverhead(!overhead)}><Eye /><span>{overhead ? '自分の視点へ' : '卓を見渡す'}</span></button></div>
    ...
    <section className="table-stage" aria-label="4人で囲むゲーム卓">
      ...
      <div className="table-tools"><button onClick={() => setDrawer('market')}><GemIcon /><span>宝石の空き目</span></button><button onClick={() => { setInspectedPlayer(opponent.id); setDrawer('collection'); }}><Trophy /><span>みんなの持ち物</span></button><button onClick={() => setDrawer('log')}><ScrollText /><span>このターンの結果</span></button></div>
      ...
      {!settled && !mining.active && !countdown && game.phase !== 'select' && game.phase !== 'inspect' && <div className="table-event" role="status"><b>OPEN!</b><span>{lastEvent?.type === 'mining' ? '宝箱から、新しいサイコロがハンターの席へ。' : lastEvent?.type === 'thief' ? '泥棒が動いた！ 宝石の行方は…' : lastEvent?.type === 'collision' ? '同じ宝石が重なった！ 宝箱へ戻ります。' : '4人の一手が、明らかに。'}</span></div>}
      {final && <div className="table-final"><Trophy /><p className="eyebrow">THE HUNT IS COMPLETE</p><h1>{winners.map(p => p.name).join(' ＆ ')}<span>{winners.length > 1 ? '同率優勝！' : 'の勝利！'}</span></h1><div className="table-ranking">{ranking.map(p => <div key={p.id}><Gem kind={p.color} /><b>{p.name}</b><small>宝石{score(p).base} ＋ ボーナス{score(p).bonus}</small><strong>{score(p, true).total}pt</strong></div>)}</div>{room.host ? <button className="primary" disabled={busy} onClick={() => void act('rematch')}>もう一度、この卓で遊ぶ<ArrowRight /></button> : <p>ホストが次のゲームを開始できます。</p>}</div>}
    </section>
    ...
    {drawer && <Drawer title={drawer === 'market' ? '宝箱と宝石マーケット' : drawer === 'collection' ? 'みんなの宝石と使用状況' : 'このターンの結果'} onClose={() => setDrawer(null)}>{drawer === 'market' ? <><p className="drawer-note">宝箱には{(game.miningCount ?? game.miningBag.length)}個のサイコロ。採掘権を使うと、ここから1個取って手持ちに加えます。</p><Market game={game} /></> : drawer === 'collection' ? <PlayerIntel game={game} selected={inspectedPlayer} opponent={opponent.id} me={room.me} onSelect={setInspectedPlayer} /> : <div className="drawer-log">{game.phase === 'select' || countdown ? <p>カップを公開し、全員が確認すると、ここに得点・泥棒・採掘などの結果が表示されます。</p> : game.phase === 'inspect' ? <p>今はカップの中身を確認する時間です。全員が「確認して処理」を押すと、得点や駒の移動が表示されます。</p> : game.logs.map((log, i) => <p key={i}>{log}</p>)}</div>}</Drawer>}
  </main>;
}
